...
#define WEBSOCKET_DEBOUNCE(f, ms)                                                               \
  {                                                                                            \
    ...
    case ClientBinaryOpcode::SERVICE_CALL_REQUEST: {
      ...
      try {
        if (!_handlers.serviceRequestHandler) {
          throw websocket::ServiceError(request.serviceId, "No service handler");
        }
...
inline void Server<ServerConfiguration>::sendMessage(ConnHandle clientHandle, ChannelId chanId,
  ...
  websocket::WriteUint32LE(msgHeader.data() + 1, subId);
  websocket::WriteUint64LE(msgHeader.data() + 5, timestamp);
...
inline void Server<ServerConfiguration>::broadcastTime(uint64_t timestamp) {
  ...
  websocket::WriteUint64LE(message.data() + 1, timestamp);
...
inline void Server<ServerConfiguration>::sendFetchAssetResponse(
  ...
  websocket::WriteUint32LE(uint32Data.data(), response.requestId);
  ...
  websocket::WriteUint32LE(uint32Data.data(), response.errorMessage.size());
  message->append_payload(uint32Data.data(), uint32Data.size());
