...
void ServiceResponse::write(uint8_t* data) const {
  size_t offset = 0;
  websocket::WriteUint32LE(data + offset, this->serviceId);
  offset += 4;
  websocket::WriteUint32LE(data + offset, this->callId);
  offset += 4;
  websocket::WriteUint32LE(data + offset, static_cast<uint32_t>(this->encoding.size()));
  offset += 4;
