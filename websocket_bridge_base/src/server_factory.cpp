...
#include <websocket_bridge/websocket_notls.hpp>
#include <websocket_bridge/websocket_server.hpp>
#include <websocket_bridge/websocket_tls.hpp>
...
std::unique_ptr<ServerInterface<websocketpp::connection_hdl>> ServerFactory::createServer(
  ...
  if (options.useTls) {
    return std::make_unique<websocket::Server<websocket::WebSocketTls>>(name, logHandler, options);
  } else {
    return std::make_unique<websocket::Server<websocket::WebSocketNoTls>>(name, logHandler, options);
  }
}
