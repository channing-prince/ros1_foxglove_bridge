...
#include <websocket_bridge/generic_service.hpp>
#include <websocket_bridge/param_utils.hpp>
#include <websocket_bridge/regex_utils.hpp>
#include <websocket_bridge/server_factory.hpp>
#include <websocket_bridge/service_utils.hpp>
...
using ClientPublications = std::unordered_map<websocket::ClientChannelId, ros::Publisher>;
using PublicationsByClient = std::map<ConnectionHandle, ClientPublications, std::owner_less<>>;
using websocket::isWhitelisted;

class WebsocketBridge : public nodelet::Nodelet {
public:
  ...
  virtual void onInit() {
    ...
    const auto send_buffer_limit = static_cast<size_t>(
      nhp.param<int>("send_buffer_limit", websocket::DEFAULT_SEND_BUFFER_LIMIT_BYTES));
    ...
    _capabilities = nhp.param<std::vector<std::string>>(
      "capabilities", std::vector<std::string>(websocket::DEFAULT_CAPABILITIES.begin(),
                                               websocket::DEFAULT_CAPABILITIES.end()));
    ...
    // ROS_INFO("Starting websocket_bridge (%s, %s@%s) with %s", rosDistro,
    //          websocket::websocket_bridge_VERSION, websocket::websocket_bridge_GIT_HASH,
    //          websocket::WebSocketUserAgent());

    try {
      websocket::ServerOptions serverOptions;
      serverOptions.capabilities = _capabilities;
      if (_useSimTime) {
        serverOptions.capabilities.push_back(websocket::CAPABILITY_TIME);
      }
      ...
      const auto logHandler =
        std::bind(&WebsocketBridge::logHandler, this, std::placeholders::_1, std::placeholders::_2);

      // Fetching of assets may be blocking, hence we fetch them in a separate thread.
      _fetchAssetQueue = std::make_unique<websocket::CallbackQueue>(logHandler, 1 /* num_threads */);

      _server = websocket::ServerFactory::createServer<ConnectionHandle>("websocket_bridge",
                                                                        logHandler, serverOptions);
      websocket::ServerHandlers<ConnectionHandle> hdlrs;
      hdlrs.subscribeHandler =
        std::bind(&WebsocketBridge::subscribe, this, std::placeholders::_1, std::placeholders::_2);
      hdlrs.unsubscribeHandler =
        std::bind(&WebsocketBridge::unsubscribe, this, std::placeholders::_1, std::placeholders::_2);
      hdlrs.clientAdvertiseHandler = std::bind(&WebsocketBridge::clientAdvertise, this,
                                               std::placeholders::_1, std::placeholders::_2);
      hdlrs.clientUnadvertiseHandler = std::bind(&WebsocketBridge::clientUnadvertise, this,
                                                 std::placeholders::_1, std::placeholders::_2);
      hdlrs.clientMessageHandler = std::bind(&WebsocketBridge::clientMessage, this,
                                             std::placeholders::_1, std::placeholders::_2);
      hdlrs.parameterRequestHandler =
        std::bind(&WebsocketBridge::getParameters, this, std::placeholders::_1,
                  std::placeholders::_2, std::placeholders::_3);
      hdlrs.parameterChangeHandler =
        std::bind(&WebsocketBridge::setParameters, this, std::placeholders::_1,
                  std::placeholders::_2, std::placeholders::_3);
      hdlrs.parameterSubscriptionHandler =
        std::bind(&WebsocketBridge::subscribeParameters, this, std::placeholders::_1,
                  std::placeholders::_2, std::placeholders::_3);
      hdlrs.serviceRequestHandler = std::bind(&WebsocketBridge::serviceRequest, this,
                                              std::placeholders::_1, std::placeholders::_2);
      hdlrs.subscribeConnectionGraphHandler = [this](bool subscribe) {
        _subscribeGraphUpdates = subscribe;
      };

      if (hasCapability(websocket::CAPABILITY_ASSETS)) {
        hdlrs.fetchAssetHandler = [this](const std::string& uri, uint32_t requestId,
                                         websocket::ConnHandle hdl) {
          _fetchAssetQueue->addCallback(
            std::bind(&WebsocketBridge::fetchAsset, this, uri, requestId, hdl));
        };
      }
      ...
      xmlrpcServer.bind("paramUpdate", std::bind(&WebsocketBridge::parameterUpdates, this,
                                                 std::placeholders::_1, std::placeholders::_2));
      xmlrpcServer.start();
...
private:
  ...
  void subscribe(websocket::ChannelId channelId, ConnectionHandle clientHandle) {
    ...
    if (it == _advertisedTopics.end()) {
      ...
      throw websocket::ChannelError(channelId, errMsg);
    }
    ...
    try {
      subscriptionsByClient.emplace(
        clientHandle, getMTNodeHandle().subscribe<ros_babel_fish::BabelFishMessage>(
                        ...
                        std::bind(&WebsocketBridge::rosMessageHandler, this, channelId, clientHandle,
                                  std::placeholders::_1)));
      if (firstSubscription) {
  ...
  void unsubscribe(websocket::ChannelId channelId, ConnectionHandle clientHandle) {
    std::lock_guard<std::mutex> lock(_subscriptionsMutex);
    ...
    if (subscriptionsIt == _subscriptions.end()) {
      throw websocket::ChannelError(channelId, "Received unsubscribe request for channel " +
  ...
  void clientAdvertise(const websocket::ClientAdvertisement& channel,
                       ConnectionHandle clientHandle) {
    if (channel.encoding != ROS1_CHANNEL_ENCODING) {
      throw websocket::ClientChannelError(
    ...
    if (publisher) {
      ...
      ROS_ERROR_STREAM(errMsg);
      throw websocket::ClientChannelError(channel.channelId, errMsg);
    }
  }

  void clientUnadvertise(websocket::ClientChannelId channelId, ConnectionHandle clientHandle) {
    std::unique_lock<std::shared_mutex> lock(_publicationsMutex);
  ...
  void clientMessage(const websocket::ClientMessage& clientMsg, ConnectionHandle clientHandle) {
    ...
    try {
      ...
      throw websocket::ClientChannelError(channelId, "Failed to publish message on topic '" +
  ...
  void updateAdvertisedTopicsAndServices(const ros::TimerEvent&) {
    ...
    const bool servicesEnabled = hasCapability(websocket::CAPABILITY_SERVICES);
    const bool querySystemState = servicesEnabled || _subscribeGraphUpdates;

    std::vector<std::string> serviceNames;
    websocket::MapOfSets publishers, subscribers, services;
    ...
    _updateTimer = getMTNodeHandle().createTimer(
      ros::Duration(nextUpdateMs / 1e3), &WebsocketBridge::updateAdvertisedTopicsAndServices, this);
  }
  ...
  void updateAdvertisedTopics() {
    ...
    std::vector<websocket::ChannelId> channelIdsToRemove;
    ...
    std::vector<websocket::ChannelWithoutId> channelsToAdd;
    for (const auto& topicAndDatatype : latestTopics) {
      ...
      websocket::ChannelWithoutId newChannel{};
  ...
  void updateAdvertisedServices(const std::vector<std::string>& serviceNames) {
    ...
    std::vector<websocket::ServiceId> servicesToRemove;
    ...
    std::vector<websocket::ServiceWithoutId> newServices;
    for (const auto& serviceName : serviceNames) {
      ...
      try {
        ...
        websocket::ServiceWithoutId service;
  ...
  void getParameters(const std::vector<std::string>& parameters,
    ...
    bool success = true;
    std::vector<websocket::Parameter> params;
  ...
  void setParameters(const std::vector<websocket::Parameter>& parameters,
                     const std::optional<std::string>& requestId, ConnectionHandle hdl) {
    using websocket::ParameterType;
  ...
  void subscribeParameters(const std::vector<std::string>& parameters,
                           websocket::ParameterSubscriptionOperation op, ConnectionHandle) {
    const auto opVerb =
      (op == websocket::ParameterSubscriptionOperation::SUBSCRIBE) ? "subscribe" : "unsubscribe";
    bool success = true;
  ...
  void logHandler(websocket::WebSocketLogLevel level, char const* msg) {
    switch (level) {
      case websocket::WebSocketLogLevel::Debug:
        ROS_DEBUG("[WS] %s", msg);
        break;
      case websocket::WebSocketLogLevel::Info:
        ROS_INFO("[WS] %s", msg);
        break;
      case websocket::WebSocketLogLevel::Warn:
        ROS_WARN("[WS] %s", msg);
        break;
      case websocket::WebSocketLogLevel::Error:
        ROS_ERROR("[WS] %s", msg);
        break;
      case websocket::WebSocketLogLevel::Critical:
        ROS_FATAL("[WS] %s", msg);
        break;
    }
  }
  ...
  void rosMessageHandler(
    const websocket::ChannelId channelId, ConnectionHandle clientHandle,
  ...
  void serviceRequest(const websocket::ServiceRequest& request, ConnectionHandle clientHandle) {
    ...
    if (serviceIt == _advertisedServices.end()) {
      ...
      ROS_ERROR_STREAM(errMsg);
      throw websocket::ServiceError(request.serviceId, errMsg);
    }
    ...
    if (!ros::service::exists(serviceName, false)) {
      throw websocket::ServiceError(request.serviceId,
  ...
  void fetchAsset(const std::string& uri, uint32_t requestId, ConnectionHandle clientHandle) {
    websocket::FetchAssetResponse response;
    ...
    try {
      ...
      response.status = websocket::FetchAssetStatus::Success;
      ...
      response.status = websocket::FetchAssetStatus::Error;
  ...
  std::unique_ptr<websocket::ServerInterface<ConnectionHandle>> _server;
  ...
  std::unordered_map<websocket::ChannelId, websocket::ChannelWithoutId> _advertisedTopics;
  std::unordered_map<websocket::ChannelId, SubscriptionsByClient> _subscriptions;
  std::unordered_map<websocket::ServiceId, websocket::ServiceWithoutId> _advertisedServices;
  ...
  std::unique_ptr<websocket::CallbackQueue> _fetchAssetQueue;
};

}  // namespace websocket_bridge

PLUGINLIB_EXPORT_CLASS(websocket_bridge::WebsocketBridge, nodelet::Nodelet)
