...
#include <websocket_bridge/test/test_client.hpp>
#include <websocket_bridge/websocket_client.hpp>
...
protected:
  void SetUp() override {
    ...
    _wsClient = std::make_shared<websocket::Client<websocketpp::config::asio_client>>();
    ASSERT_EQ(std::future_status::ready, _wsClient->connect(URI).wait_for(DEFAULT_TIMEOUT));
  }

  ros::NodeHandle _nh;
  std::shared_ptr<websocket::Client<websocketpp::config::asio_client>> _wsClient;
};
...
TEST(SmokeTest, testConnection) {
  websocket::Client<websocketpp::config::asio_client> wsClient;
...
TEST(SmokeTest, testSubscription) {
  ...
  for (auto i = 0; i < clientCount; ++i) {
    ...
    auto client = std::make_shared<websocket::Client<websocketpp::config::asio_client>>();
    auto channelFuture = websocket::waitForChannel(client, topic_name);
    ...
    const websocket::Channel channel = channelFuture.get();
    const websocket::SubscriptionId subscriptionId = 1;
...
TEST(SmokeTest, testSubscriptionParallel) {
  ...
  const websocket::SubscriptionId subscriptionId = 1;
  auto clients = {
    std::make_shared<websocket::Client<websocketpp::config::asio_client>>(),
    std::make_shared<websocket::Client<websocketpp::config::asio_client>>(),
    std::make_shared<websocket::Client<websocketpp::config::asio_client>>(),
  };
...
TEST(SmokeTest, testPublishing) {
  ...
  websocket::ClientAdvertisement advertisement;
...
TEST_F(ParameterTest, testGetAllParams) {
  ...
  auto future = websocket::waitForParameters(_wsClient, requestId);
  _wsClient->getParameters({}, requestId);
  ASSERT_EQ(std::future_status::ready, future.wait_for(DEFAULT_TIMEOUT));
  std::vector<websocket::Parameter> params = future.get();
...
TEST_F(ParameterTest, testGetParameters) {
  ...
  const auto array_params = p2Iter->getValue().getValue<std::vector<websocket::ParameterValue>>();
...
TEST_F(ParameterTest, testSetParameters) {
  ...
  const std::vector<websocket::ParameterValue> newP2value = {4.1, 5.5, 6.6};

  const std::vector<websocket::Parameter> parameters = {
    websocket::Parameter(PARAM_1_NAME, newP1value),
    websocket::Parameter(PARAM_2_NAME, newP2value),
  };
...
TEST_F(ParameterTest, testUnsetParameter) {
  const std::vector<websocket::Parameter> parameters = {
    websocket::Parameter(PARAM_1_NAME),
  };
...
TEST_F(ParameterTest, testParameterSubscription) {
  auto future = websocket::waitForParameters(_wsClient);

  _wsClient->subscribeParameterUpdates({PARAM_1_NAME});
  _wsClient->setParameters({websocket::Parameter(PARAM_1_NAME, "foo")});
  ASSERT_EQ(std::future_status::ready, future.wait_for(DEFAULT_TIMEOUT));
  std::vector<websocket::Parameter> params = future.get();
  ...
  _wsClient->setParameters({websocket::Parameter(PARAM_1_NAME, "bar")});

  future = websocket::waitForParameters(_wsClient);
...
TEST_F(ParameterTest, testGetParametersParallel) {
  ...
  std::vector<std::future<std::vector<websocket::Parameter>>> futures;
  for (auto client : clients) {
    futures.push_back(
      std::async(std::launch::async, [client]() -> std::vector<websocket::Parameter> {
        if (std::future_status::ready == client->connect(URI).wait_for(DEFAULT_TIMEOUT)) {
          const std::string requestId = "req-123";
          auto future = websocket::waitForParameters(client, requestId);
  ...
  for (auto& future : futures) {
    ASSERT_EQ(std::future_status::ready, future.wait_for(DEFAULT_TIMEOUT));
    std::vector<websocket::Parameter> parameters;
...
TEST_F(ServiceTest, testCallServiceParallel) {
  ...
  auto serviceFuture = websocket::waitForService(*clients.begin(), SERVICE_NAME);
  for (auto client : clients) {
  ...
  const websocket::Service service = serviceFuture.get();

  websocket::ServiceRequest request;
  ...
  std::vector<std::future<websocket::ServiceResponse>> futures;
  for (auto client : clients) {
    futures.push_back(websocket::waitForServiceResponse(client));
    client->sendServiceRequest(request);
  }

  for (auto& future : futures) {
    ASSERT_EQ(std::future_status::ready, future.wait_for(std::chrono::seconds(5)));
    websocket::ServiceResponse response;
...
TEST(FetchAssetTest, fetchExistingAsset) {
  auto wsClient = std::make_shared<websocket::Client<websocketpp::config::asio_client>>();
  EXPECT_EQ(std::future_status::ready, wsClient->connect(URI).wait_for(DEFAULT_TIMEOUT));
  ...
  auto future = websocket::waitForFetchAssetResponse(wsClient);
  wsClient->fetchAsset(uri, requestId);
  ASSERT_EQ(std::future_status::ready, future.wait_for(DEFAULT_TIMEOUT));
  const websocket::FetchAssetResponse response = future.get();

  EXPECT_EQ(response.requestId, requestId);
  EXPECT_EQ(response.status, websocket::FetchAssetStatus::Success);
...
TEST(FetchAssetTest, fetchNonExistingAsset) {
  ...
  EXPECT_EQ(response.requestId, requestId);
  EXPECT_EQ(response.status, websocket::FetchAssetStatus::Error);
